...
export type AttendanceMark = 'P' | 'A' | 'L'; // P = Present (Green), A = Absent (Red), L = Late (Yellow)
...
export interface AttendanceSession {
  ...
  subject: string; // 'Induction', 'HTML & LAB', 'JAVASCRIPT', etc.
}
...
export interface DailyTimeLog {
  ...
  studentName: string;
  date: string;
  inTime: string; // e.g. "09:10"
  outTime: string; // e.g. "17:30"
  status: 'P' | 'L' | 'A';
...
export interface StudentOutcome {
  ...
  // 9-Month Course Completion & Graduate Career Outcome Fields
  currentStatus?: string; // e.g. 'Unemployed', 'Employed', 'Internship'
  workingCompanyName?: string; // e.g. 'Virtusa', 'WSO2', 'N/A'
  salary?: number; // e.g. 85000, 0
  courseCompletionStatus?: string; // e.g. 'Completed (9 Months)', 'Graduated with Merit'
  courseSpecialization?: string; // e.g. 'Software Development (Full-Stack Web)'
  employmentStatus?: string; // e.g. 'Employed (Full-Time)', 'Internship', 'Unemployed'
  otherStatus?: string; // e.g. 'Seeking Placement Assistance', 'None'
  workLocation?: string; // e.g. 'Colombo', 'Jaffna', 'Remote'
  linkedinUrl?: string;
  contactPhone?: string;
  contactEmail?: string;
}
